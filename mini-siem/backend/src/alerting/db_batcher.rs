...
async fn flush_batch(db: &PostgresDb, buffer: &mut Vec<Alert>) -> anyhow::Result<()> {
    ...
    // Aggregate unique source_ips
    let ips: Vec<String> = buffer.iter().map(|a| a.source_ip.clone()).collect();
    let unique_ips: Vec<String> = {
        let mut s = ips.clone();
        s.sort(); s.dedup(); s
    };

    // Fetch existing open alerts for these IPs in one query
    let existing = db.get_open_alerts_by_ips(&unique_ips).await.unwrap_or_default();

    // Map by source_ip to existing alert (take first)
    ...
    // Batch update existing alerts using upsert to reduce DB roundtrips
